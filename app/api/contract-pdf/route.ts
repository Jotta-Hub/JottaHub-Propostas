...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .sigs-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 32px; }
  .sig-box {
    background: #111;
    border: 1px solid #1C1C1C;
    border-radius: 3px;
...
<div class="title-block">
  <div class="doc-type">Proposta Comercial — Documento Final</div>
  <h1>${proposal.client}<br><span>${proposal.title || 'Projeto'}</span></h1>
  <div class="subtitle">Emitido em ${fmtDate(proposal.created_at)}</div>
</div>

<div class="section">
  <div class="section-label">Dados da Proposta</div>
  <div class="row"><span class="row-key">Cliente</span><span class="row-val">${proposal.client}</span></div>
  ${proposal.contact ? `<div class="row"><span class="row-key">Contato</span><span class="row-val">${proposal.contact}</span></div>` : ''}
  <div class="row"><span class="row-key">Projeto</span><span class="row-val">${proposal.title || '—'}</span></div>
  <div class="row"><span class="row-key">Data de Emissão</span><span class="row-val">${fmtDate(proposal.created_at)}</span></div>
